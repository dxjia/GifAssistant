...
public class GifMerger {
    ...
    public static boolean generateGifProduct(String output, String sourceFile, int ss, int se) {
        if (mFFmpegNativeHelper.ffmpegRunCommand(getffmpegCommand(output, sourceFile, ss, se)) == 0) {
            return false;
        } else {
            return true;
        }
     ...
     */
    private static String getffmpegCommand(String output, String sourceFile, int ss, int se) {
        String command = "";
