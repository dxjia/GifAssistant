...
package com.chenupt.springindicator;
...
public class SpringIndicator extends FrameLayout {
	...
	private com.ryanharter.viewpager.ViewPager viewPager;
	...
	public void onPageScrolled(int position, float positionOffset,
		...
		if (position < tabs.size() - 1) {
			...
			if (positionOffset < radiusOffsetFoot) {
				springView.getFootPoint().setRadius(
						(1 - positionOffset / radiusOffsetFoot) * radiusOffset
								+ radiusMin);
			} else {
		...
		if (delegateListener != null) {
			delegateListener.onPageScrolled(position, positionOffset,
